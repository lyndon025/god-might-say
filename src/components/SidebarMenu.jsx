...
const SidebarMenu = () => {
    const { isMenuOpen, user, signInWithGoogle, logOut, setPage, setIsMenuOpen, deleteChatHistory } = useContext(AppContext);
    ...
    return (
        <div className={`fixed top-0 right-0 h-full bg-surface border-l-2 border-accent/20 shadow-2xl z-40 transform transition-transform duration-300 ease-in-out ${isMenuOpen ? 'translate-x-0' : 'translate-x-full'} w-72`}>
            <div className="absolute top-4 right-4">
                <button onClick={() => setIsMenuOpen(false)} className="p-2 rounded-full text-accent hover:bg-accent/10 transition-colors">
                    <X size={24} />
                </button>
            </div>
    ...
    <div className="mt-4 border-t border-accent/10"></div>
                ...
                <button onClick={() => handleNavigation('donate')} className="flex items-center w-full text-left p-3 text-lg rounded-lg text-primary-text hover:bg-accent/10 transition-colors mt-4">
                    <Heart className="mr-4 text-accent" size={20} /> Donate
                </button>

                {/* The "About" button is now second. */}
                <button onClick={() => handleNavigation('about')} className="flex items-center w-full text-left p-3 text-lg rounded-lg text-primary-text hover:bg-accent/10 transition-colors">
                    <Info className="mr-4 text-accent" size={20} /> About
                </button>
